...
- [x] **Centralize Date Utilities** — Create `@/lib/utils/date.ts` to handle all UTC midnight normalization, ISO slicing, and formatting. Replace all scattered `new Date(Date.UTC(...))` and `toISOString().slice(0,10)` calls.
