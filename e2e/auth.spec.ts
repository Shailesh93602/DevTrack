...
test.describe("Authentication Flows", () => {
  test("[SCREENSHOT-01] should show login page with all elements", async ({
    page,
  }) => {
  ...
  test("[SCREENSHOT-02] should show validation errors for empty fields", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.waitForLoadState("load");
  ...
  test("[SCREENSHOT-03] should show validation error for invalid email format", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.waitForLoadState("load");
  ...
  test("[SCREENSHOT-04] should show password requirements on signup page", async ({
    page,
  }) => {
  ...
  test("[SCREENSHOT-05] should show validation error for weak password on signup", async ({
    page,
  }) => {
    await page.goto("/signup");
    await page.waitForLoadState("load");
...
test.describe("Registration and Login Integration", () => {
  ...
  test("[SCREENSHOT-06] should sign in with pre-created user and reach dashboard", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.waitForLoadState("load");
  ...
  test("[SCREENSHOT-07] should login with same credentials and redirect to dashboard", async ({
    page,
  }) => {
    ...
    console.log(
      "✅ Login successful - Screenshot saved to test-results/07-dashboard-after-login.png"
    );
  });

  test("[SCREENSHOT-08] should show error for non-existent user login", async ({
    page,
  }) => {
    await page.goto("/login");
    await page.waitForLoadState("load");
    ...
    await expect(
      page.getByText(/invalid|not found|credentials|error/i)
  ...
  test("should toggle password visibility", async ({ page }) => {
    ...
    const showButton = page
      .locator('button[aria-label*="show"], button:has([data-lucide="eye"])')
      .first();
